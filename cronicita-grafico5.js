...
function extractTotal(dataNew, column, sex = "Maschi") {
    let dataset = null;
    let dataRif = null;
    let sum = 0;
    const references = dataNew.reduce(
        (l, i) => (l.indexOf(i.riferimento) !== -1 ? l : l.concat([i.riferimento])),
        []
    );
    if (references.length > 1 && column === "peso_classe") {
        dataRif = dataNew.filter((d) => d.riferimento === dataNew[0].riferimento);
    } else {
        dataRif = dataNew;
    }
    dataset = dataRif.filter((d) => d.sesso === sex);
